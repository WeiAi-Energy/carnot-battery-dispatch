...
k_charge = 302.8619552671795  # Factor for converting mass flow to charge power
k_discharge = 204.3253246362388  # Factor for converting mass flow to discharge power
...
def evaluate_candidate(args):
    ...
    obj = -(total_profit - total_penalty - mass_penalty)
    return (y, obj, total_profit, total_penalty)


# Class for passing data between processes
class OptimizationParameters:
    def __init__(self, MILP_modes, charge_indices, discharge_indices, num_charge, num_discharge,
                 lower_bounds, upper_bounds, mass_norm, store_temps, lmps, ptes,
                 MILP_m_norms, MILP_beta_norms, is_final_day):
...
def expand_parameters(x_compact, params):
    """
    Expand compact parameters to full parameter vectors with fixed charging beta
    """
    num_hours = len(params.MILP_modes)
    m_norms_full = np.zeros(num_hours)
    beta_norms_full = np.zeros(num_hours)

    charge_m_norms = x_compact[:params.num_charge]
    discharge_m_norms = x_compact[params.num_charge:params.num_charge + params.num_discharge]
    discharge_beta_norms = x_compact[params.num_charge + params.num_discharge:]

    for i, idx in enumerate(params.charge_indices):
        m_norms_full[idx] = charge_m_norms[i]
        beta_norms_full[idx] = 1.05
...
def optimize_operation_hybrid(mass_norm, store_temps, lmps, ptes, is_final_day=False):
    """
    ...
    charge_indices = [i for i, mode in enumerate(MILP_modes[:24]) if mode == -1]
    discharge_indices = [i for i, mode in enumerate(MILP_modes[:24]) if mode == 1]
    num_charge = len(charge_indices)
    num_discharge = len(discharge_indices)

    print(
        f"Optimizing {num_charge} charging time steps and {num_discharge} discharging time steps out of 24 (first day only)")

    # If no non-idle time steps, just return the MILP solution
    if num_charge + num_discharge == 0:
        print("No non-idle time steps to optimize. Using MILP solution directly.")
        df, _, _, _, _ = compute_one_horizon(
            MILP_modes, MILP_m_norms, MILP_beta_norms,
            mass_norm, store_temps, lmps, ptes
        )
        return df, MILP_modes, MILP_m_norms, MILP_beta_norms

    # Define bounds for optimization with more flexibility for m_norm
    m_norm_lower = MFR_LOWER
    ...
    lower_bounds = np.array(
        [m_norm_lower] * num_charge + [m_norm_lower] * num_discharge + [beta_norm_lower] * num_discharge)
    upper_bounds = np.array(
        [m_norm_upper] * num_charge + [m_norm_upper] * num_discharge + [beta_norm_upper] * num_discharge)
    ...
    initial_charge_m_norms = np.array([MILP_m_norms[i] for i in charge_indices])
    initial_discharge_m_norms = np.array([MILP_m_norms[i] for i in discharge_indices])
    initial_discharge_beta_norms = np.array([MILP_beta_norms[i] for i in discharge_indices])
...
def plot_results(days, results, price_data=None, output_dir="./results_minlp_pricetaker1"):
    ...
    # Create a figure with multiple subplots (3 instead of 4)
    ...
    xtick_positions = range(0, 24 * days + 1, 24)  # 0, 24, 48, ..., 168
    ...
    plt.savefig(f"{output_dir}/minlp_pricetaker_thh.png", dpi=600, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    import os
    results_dir = "./results_minlp_pricetaker_t_hh_max"
