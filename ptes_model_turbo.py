...
class PTES_our:
    ...
    def find_alpha_values(self, phase: str, m, beta, t_com, t_exp, p_exp, n):
        """
        Find the optimal alpha_com and alpha_exp values to match the desired beta

        Returns:
            tuple: (alpha_com, alpha_exp, vio_beta_com, vio_beta_exp)
                - alpha_com: optimal compressor alpha value
                - alpha_exp: optimal expander alpha value
                - vio_beta_com: absolute difference between achieved beta_com and target beta
                - vio_beta_exp: absolute difference between achieved beta_exp and target beta
        """
        p_com = p_exp / beta

        # --- Compressor (alpha_com) calculation ---
        if phase == "char":
            ...
            beta_com_dot = beta / self.beta_char_0
            beta_0 = self.beta_char_0
            # For expander calculation
            ...
            beta_com_dot = beta / self.beta_dis_0
        ...
        # Calculate normalized speed parameters for compressor
        n_com_bar = n_com / np.sqrt(t_com)
        n_com_dot = n_com_bar / n_com_bar_0

        # Calculate pressure and flow parameters for compressor
        ...
        # Define a function to calculate beta_com for a given alpha_com (directly embedded)
        ...
        if discriminant >= 0:
            ...
            # Check if only one solution is in the valid range [-20, 60]
            ...
            if valid1 and not valid2:
                ...
                # If both or neither are valid, find which one (including boundaries) gives closest beta_com
                candidates = [-20, 60]
                best_alpha = None
                min_diff = float('inf')
        ...
        # --- Expander (alpha_exp) calculation ---
        # Calculate pressure and flow parameters for expander
        G_exp_bar = m * np.sqrt(t_exp) / p_exp
        G_exp_dot = G_exp_bar / G_exp_bar_0

        # Define function to calculate beta_exp for a given alpha_exp (directly embedded)
        def calculate_beta_exp(alpha_exp_val):
            G_exp_dot_map = G_exp_dot / (1.1875 - 3 / 10000 * (alpha_exp_val - 25) ** 2)
            speed_factor = np.sqrt(1.4 - 0.4 * n_exp / self.n_0)
            temp_factor = np.sqrt(t_exp_ref / t_exp)

            return np.sqrt((G_exp_dot_map / speed_factor / temp_factor) ** 2 * beta_exp_factor + 1)

        # Solve for G_exp_dot_map that would give beta_exp = beta
        speed_factor = np.sqrt(1.4 - 0.4 * n_exp / self.n_0)
        temp_factor = np.sqrt(t_exp_ref / t_exp)

        # From beta_exp = sqrt((G_exp_dot_map / speed_factor / temp_factor)^2 * beta_exp_factor + 1)
        # Calculate required G_exp_dot_map
        required_G_exp_dot_map = speed_factor * temp_factor * np.sqrt((beta ** 2 - 1) / beta_exp_factor)

        # From G_exp_dot_map = G_exp_dot / (1.1875 - 3/10000 * (alpha_exp - 25)^2)
        # Solve for alpha_exp
        # (1.1875 - 3/10000 * (alpha_exp - 25)^2) * required_G_exp_dot_map = G_exp_dot

        # Calculate discriminant for alpha_exp quadratic equation
        ...
        if exp_discriminant >= 0:
            ...
            # Check if only one solution is in the valid range [-30, 25]
            valid1 = -30 <= alpha_exp1 <= 25

            if valid1:
                alpha_exp = alpha_exp1
            else:
                # If both or neither are valid, find which one (including boundaries) gives closest beta_exp
                candidates = [-30, 25]
                best_alpha = None
                min_diff = float('inf')
    ...
    def exloss(self, phase: str, m, beta, t_com, t_exp, p_exp, n):

        # Calculate normalized parameters based on the operation phase
        if phase == "char":
            # Charging phase parameter initialization
            n_com_bar_0 = self.n_0 / np.sqrt(self.t_com_char_0)
        ...
        # Calculate normalized speed parameters
        n_com_bar = n_com / np.sqrt(t_com)
        n_exp_bar = n_exp / np.sqrt(t_exp)
        n_com_dot = n_com_bar / n_com_bar_0
        ...
        alpha_com, alpha_exp = self.find_alpha_values(phase, m, beta, t_com, t_exp, p_exp, n)
        ...
        if phase == "char":
            beta_com_map = (c1 * G_com_dot_map ** 2 + c2 * G_com_dot_map + c3) * self.beta_char_0
        else:  # discharge
            beta_com_map = (c1 * G_com_dot_map ** 2 + c2 * G_com_dot_map + c3) * self.beta_dis_0
        ...
        # Calculate expander map parameters and efficiency
        G_exp_dot_map = G_exp_dot / (1.1875 - 3 / 10000 * (alpha_exp - 25) ** 2)

        if phase == "char":
            beta_exp = np.sqrt(
                (G_exp_dot_map / np.sqrt(1.4 - 0.4 * n_exp / self.n_0) / np.sqrt(self.t_exp_char_0 / t_exp)) ** 2 * (
                        self.beta_char_0 ** 2 - 1) + 1)
        else:
            beta_exp = np.sqrt(
                (G_exp_dot_map / np.sqrt(1.4 - 0.4 * n_exp / self.n_0) / np.sqrt(self.t_exp_dis_0 / t_exp)) ** 2 * (
                        self.beta_dis_0 ** 2 - 1) + 1)
        ...
        vio_beta_com = abs(beta_com - beta)
        vio_beta_exp = abs(beta_exp - beta)

        return exloss_norm, eff_com, eff_exp, alpha_com, alpha_exp, vio_choke_com, vio_choke_exp, vio_surge, vio_beta_com, vio_beta_exp

    def opt_control(self, phase: str, m_actual: float, beta: float, t_com: float, t_exp: float, x_pre=None) -> tuple:
        """
        Optimize control parameters for the system.
        Modified to be consistent with the improved calculate_performance method.
        """
        ...
        def objective(vars):
            ...
            exloss_norm, eff_com, eff_exp, alpha_com, alpha_exp, vio_choke_com, vio_choke_exp, vio_surge, vio_beta_com, vio_beta_exp = self.exloss(
                    phase, m_actual, beta, t_com, t_exp, p_exp, n)

            # Add penalty term to avoid constraint violations (excluding temperature violations)
            penalty = 0.1 * penalty_factor * np.sqrt(vio_choke_com ** 2 + vio_choke_exp ** 2 + vio_surge ** 2
                                               + (vio_beta_com ** 2 + vio_beta_exp ** 2))
        ...
        # n_values = [0.8, 0.85, 0.9, 0.95, 1.0]
        # p_exp_values = np.clip([0.85 * p_exp_initial, p_exp_initial, 1.15 * p_exp_initial], [0.1, 0.1, 0.1], [1, 1, 1])
        n_values = [0.95]
        p_exp_values = np.clip([p_exp_initial, p_exp_initial * 1.15], [0.1, 0.1], [1, 1])
        ...
        return n, p_exp, eff_com, eff_exp, alpha_com, alpha_exp, vio_choke_com, vio_choke_exp, vio_surge, vio_beta_com, vio_beta_exp
    ...
    def calculate_performance(self, phase: str, m_norm: float, beta_norm: float, current_temps: dict) -> dict:
        ...
        while (error > tol or beta_flag == 1) and iter_count < max_iter:
            if phase == "char":
                ...
                if error < 1:
                    n, p_exp, eff_com, eff_exp, alpha_com, alpha_exp, vio_choke_com, vio_choke_exp, vio_surge, vio_beta_com, vio_beta_exp = self.opt_control(
        ...
        return {
            ...
            'vio_surge': vio_surge,
            'vio_beta': vio_beta_com + vio_beta_exp,
            'eff_ex': eff_ex
        }